...
InfIntException::InfIntException(const std::string& txt) throw() : std::exception(), txt(txt) {}

InfIntException::~InfIntException() throw() {}

const char* InfIntException::what() const throw() { return txt.c_str(); }
#endif
...
inline InfInt::InfInt() : pos(true) { val.push_back((ELEM_TYPE)0); }

inline InfInt::InfInt(const char* c) { fromString(c); }

inline InfInt::InfInt(const std::string& s) { fromString(s); }
...
inline InfInt InfInt::operator-() const {  // PROFILED_SCOPE
    InfInt result = *this;
    result.pos = !pos;
    return result;
}

inline InfInt InfInt::operator+(const InfInt& rhs) const {  // PROFILED_SCOPE
    InfInt result;
...
inline InfInt InfInt::operator*(const InfInt& rhs) const {  // PROFILED_SCOPE
    ...
    for (;; ++digit) {  // PROFILED_SCOPE
        ...
        for (size_t i = digit < rhs.val.size() ? 0 : digit - rhs.val.size() + 1;
             i < val.size() && i <= digit; ++i) {  // PROFILED_SCOPE
            PRODUCT_TYPE pval = result.val[digit] + val[i] * (PRODUCT_TYPE)rhs.val[digit - i];
            if (pval >= BASE || pval <= -BASE) {  // PROFILED_SCOPE
                // carry += pval / BASE;
                // pval %= BASE;
        ...
        if (!found) {  // PROFILED_SCOPE
            break;
        }
    }
    for (; carry > 0; ++digit) {  // PROFILED_SCOPE
        result.val[digit] = (ELEM_TYPE)(carry % BASE);
        carry /= BASE;
    }
    result.correct();
...
inline InfInt InfInt::operator/(const InfInt& rhs) const {  // PROFILED_SCOPE
    ...
    for (int i = (int)N.val.size() - 1; i >= 0; --i) {  // PROFILED_SCOPE
        R.val.insert(R.val.begin(), (ELEM_TYPE)0);
        R.val[0] = N.val[i];
        R.correct(true);
        ELEM_TYPE cnt = dInR(R, D);
...
inline InfInt InfInt::operator*(ELEM_TYPE rhs) const {  // PROFILED_SCOPE
    InfInt result = *this;
    ELEM_TYPE factor = rhs < 0 ? -rhs : rhs;
...
inline bool InfInt::operator==(const InfInt& rhs) const {  // PROFILED_SCOPE
...
inline void InfInt::optimizeSqrtSearchBounds(InfInt& lo, InfInt& hi) const {  // PROFILED_SCOPE
    InfInt hdn = one;
...
inline InfInt InfInt::intSqrt() const {  // PROFILED_SCOPE
    if (*this <= zero) {
#ifdef INFINT_USE_EXCEPTIONS
...
inline char InfInt::digitAt(size_t i) const {  // PROFILED_SCOPE
    if (numberOfDigits() <= i) {
#ifdef INFINT_USE_EXCEPTIONS
...
inline size_t InfInt::numberOfDigits() const {  // PROFILED_SCOPE
...
inline std::string InfInt::toString() const {  // PROFILED_SCOPE
    std::ostringstream oss;
    oss << *this;
    return oss.str();
}

inline size_t InfInt::size() const {  // PROFILED_SCOPE
    return val.size() * sizeof(ELEM_TYPE) + sizeof(bool);
}

inline int InfInt::toInt() const {  // PROFILED_SCOPE
...
inline long InfInt::toLong() const {  // PROFILED_SCOPE
...
inline long long InfInt::toLongLong() const {  // PROFILED_SCOPE
...
inline unsigned int InfInt::toUnsignedInt() const {  // PROFILED_SCOPE
...
inline unsigned long InfInt::toUnsignedLong() const {  // PROFILED_SCOPE
...
inline unsigned long long InfInt::toUnsignedLongLong() const {  // PROFILED_SCOPE
...
inline void InfInt::truncateToBase() {  // PROFILED_SCOPE
    for (size_t i = 0; i < val.size(); ++i)  // truncate each
    {
        if (val[i] >= BASE || val[i] <= -BASE) {  // PROFILED_SCOPE
            div_t dt = div(val[i], BASE);
            val[i] = dt.rem;
            if (i + 1 >= val.size()) {  // PROFILED_SCOPE
...
inline bool InfInt::equalizeSigns() {  // PROFILED_SCOPE
...
inline void InfInt::removeLeadingZeros() {  // PROFILED_SCOPE
...
inline void InfInt::correct(bool justCheckLeadingZeros, bool hasValidSign) {  // PROFILED_SCOPE
...
inline void InfInt::fromString(const std::string& s) {  // PROFILED_SCOPE
    pos = true;
    val.clear();
    // TODO use resize
...
inline ELEM_TYPE InfInt::dInR(const InfInt& R, const InfInt& D) {  // PROFILED_SCOPE
    ...
    while (max - min > 0) {
        ...
        InfInt prod = D * avg;
        if (R == prod) {  // PROFILED_SCOPE
            return avg;
        } else if (R > prod) {  // PROFILED_SCOPE
            min = avg;
        } else {  // PROFILED_SCOPE
            max = avg - 1;
        }
...
inline void InfInt::multiplyByDigit(ELEM_TYPE factor,
                                    std::vector<ELEM_TYPE>& val) {  // PROFILED_SCOPE
    ELEM_TYPE carry = 0;
    for (size_t i = 0; i < val.size(); ++i) {
...
inline std::istream& operator>>(std::istream& s, InfInt& n) {  // PROFILED_SCOPE
    std::string str;
    s >> str;
    n.fromString(str);
    return s;
...
inline std::ostream& operator<<(std::ostream& s, const InfInt& n) {  // PROFILED_SCOPE
    if (!n.pos) {
        s << '-';
    }
    bool first = true;
